...
contract DEX {
	...
	function price(
		...
		uint256 xAdjustedInput = xInput * (1000 - 3) / 1000;
		return yReserves * xAdjustedInput / (xReserves + xAdjustedInput);
	}
	...
	function ethToToken() public payable returns (uint256 tokenOutput) {
		...
		uint256 dy = price(msg.value, xReserves, yReserves);
		require(token.transfer(msg.sender, dy), "ethToToken(): reverted swap.");
		emit EthToTokenSwap(msg.sender, dy, msg.value);
		return dy;
	...
	function tokenToEth(
		...
		uint256 ethSend = price(tokenInput, xReserves, yReserves);
		require(token.transferFrom(msg.sender, address(this), tokenInput), "tokenToEth(): Transfer Failed BAL");
		(bool sent, ) = msg.sender.call{ value: ethSend }("");
        require(sent, "tokenToEth: revert in transferring eth to you!");
		emit TokenToEthSwap(msg.sender, tokenInput, ethSend);
		return ethSend;
	...
	function deposit() public payable returns (uint256 tokensDeposited) {
		...
		uint256 xReserves = address(this).balance - msg.value;
		uint256 yReserves = token.balanceOf(address(this));
		uint256 balAmount = msg.value * yReserves / totalLiquidity + 1;

		uint256 mintedLiquidity = msg.value * totalLiquidity / xReserves;
		...
		emit LiquidityProvided(msg.sender, mintedLiquidity, msg.value, balAmount);
	...
	function withdraw(
		...
		require(amount <= liquidity[msg.sender], "withdraw(): Insificient liquidity provided");
		uint256 ethReserve = address(this).balance;
		uint256 balReserve = token.balanceOf(address(this));
		uint256 ethWithdraw = amount * ethReserve / totalLiquidity;

		uint256 balWithdraw = amount * balReserve / totalLiquidity;
		liquidity[msg.sender] -= amount;
		totalLiquidity -= amount;
		(bool sent,) = payable(msg.sender).call{ value: ethWithdraw }("");
		require(sent, "withdraw(): Eth sent failed");
		require(token.transfer(msg.sender, balWithdraw), "withdraw(): bal sent failed");
		emit LiquidityRemoved(msg.sender, amount, balWithdraw, ethWithdraw);
		return (ethWithdraw, balWithdraw);
	}
}
